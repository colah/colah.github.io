...
function to_probs(l) {
  l = l.map(p => Math.max(p, 1e-3));
  var norm = reduce_sum(l);
  if (norm < 1){
    return l.map(p => p + (1-norm)/l.length);
  } else {
    return l.map(p => p/norm);
  }
}

function to_probs2(l) {
  var eps = 1e-4;
  l = l.map(p => Math.max(p, eps));
  var mask = l.map(p => (p <= eps)? 0 : 1);
  var norm = reduce_sum(l);
  if (norm < 1){
    return l.map(p => p + (1-norm)/l.length);
  } else if (norm > 1 + eps) {
    var d = (norm-1)/reduce_sum(mask);
    return to_probs(_.range(l.length).map(n => l[n] - d*mask[n]));
  } else {
    return l;
  }
}
